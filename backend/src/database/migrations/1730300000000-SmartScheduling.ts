...
/**
 * Умное расписание: навыки мастеров, графики работы, плановое/фактическое время заказов.
 */
export class SmartScheduling1730300000000 implements MigrationInterface {
  name = 'SmartScheduling1730300000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`CREATE EXTENSION IF NOT EXISTS "uuid-ossp"`);

    await queryRunner.query(
      `ALTER TABLE "staff_members" ADD COLUMN IF NOT EXISTS "skills" jsonb NOT NULL DEFAULT '[]'::jsonb`,
    );
    ...
    await queryRunner.query(
      `UPDATE "orders" SET planned_start_time = date_time, planned_end_time = date_time + interval '1 hour' WHERE planned_start_time IS NULL`,
    );

    await queryRunner.query(
      `ALTER TABLE "order_items" ADD COLUMN IF NOT EXISTS "master_id" uuid REFERENCES "staff_members"("id") ON DELETE SET NULL`,
    );
  }
