/**
 * Dev-only: найти чаты с одинаковыми (organization_id, client_phone) для последующего слияния.
 * Запуск: npx ts-node -r tsconfig-paths/register scripts/chats-find-duplicates.ts
 */
...
async function main() {
  const ds = new DataSource({
    ...
    database: process.env.DB_DATABASE || 'mp_servis',
    entities: [],
    synchronize: false,
  });
  await ds.initialize();
