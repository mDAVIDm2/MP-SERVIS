...
echo ========================================
echo   MP-Servis Client - phone / emulator
echo ========================================
echo.
...
echo.
echo Если установка падает ^(подпись не совпадает^): удалите приложение Клиент
echo с телефона или выполните: adb uninstall ru.mpservis.client
echo.

cd /d "%~dp0autohub_client2"
if errorlevel 1 (
  echo ERROR - Cannot cd to autohub_client2.
  set "RUN_EXIT=1"
  goto :final
)
if not exist "pubspec.yaml" (
  echo ERROR - pubspec.yaml missing in autohub_client2.
  set "RUN_EXIT=1"
  goto :final
)
